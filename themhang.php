...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '    <div class="h5">' .$sl. '</div>';
        echo '    <div class="h5">' .$tensanpham . '</div>';
        echo '<div class="h5">' .$g . '<sup>đ</sup><div>'  . $ngay . '</div></div>';
        echo '  </div>';
    }
} else {
...
if (mysqli_num_rows($result3) > 0) {
    while ($row = mysqli_fetch_assoc($result3)) {
        ...
        echo '<div>';
        echo '<h1>' . $ten . '</h1>';
        echo '<h2>Giá: ' . $gia_giam . '<sup>đ</sup></h2>';//(empty($t) ? $gia_giam : "")
        ...
        echo '</div>';
        echo '<h2>Tổng giá: <span id="totalPrice">' . $gia_giam . '<sup>đ</sup></span></h2>';
